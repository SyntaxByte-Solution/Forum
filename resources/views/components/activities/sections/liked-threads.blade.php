<div class="activities-section activities-liked-threads-section">
    <div class="activities-section-content-header flex">
        <h3 class="move-to-middle forum-color flex align-center unselectable">
            ...
            {{ __('Liked discussions') }} [<span class="current-section-thread-count mx4">{{ $likedthreads->count() }}</span> / {{ $likedthreadscount }} ]
        </h3>
    </div>
    <div class="activities-section-content">
        ...
        @if(!$likedthreads->count())
            <div class="full-center" style="margin: 46px auto 0 auto; width: 80%;">
                <div class="flex flex-column align-center">
                    ...
                    @if(\Illuminate\Support\Facades\Auth::check() && auth()->user()->id == $user->id)
                        <p class="fs20 text-center bold my8 text-center">{{ __("You don't have any liked discussions for the moment") }} !</p>
                        <div class="flex">
                            <div class="flex align-center move-to-middle my4">
                                <p class="text-center no-margin text-center">{{ __("Eplore other peoples' discussions and hit like button to interact with other people posts") }}</p>
                            </div>
                        </div>
                    @else
                        <p class="fs20 text-center bold my8 text-center">{{ __("This user has no liked discussions for the moment") }} !</p>
                    @endif
                </div>
            </div>
        @endif
