    ...
    <a href="{{ $action_resource_link }}" class="link-wraper notification-component-container" style="@if(!$notif_read) background-color:#ccebf74a @endif">
        <div class="relative" style="height: max-content">
            <div class="size48 rounded hidden-overflow mr8 relative">
                ...
                <img data-src="{{ $action_user->sizedavatar(36, '-l') }}" class="action_takers_image lazy-image image-with-fade handle-image-center-positioning" alt="{{ $action_user->firstname . ' ' . $action_user->lastname . ' profile picture' }}">
            </div>
            <div class="action_type_icon notification-type-icon sprite sprite-2-size {{ $resource_action_icon }}"></div>
        </div>
        <div>
            <div class="fs14">
                <strong class="action_takers">{{ $action_takers }}</strong> <span class="action_statement bblack">{{ $action_statement }}</span> <span class="action_resource_slice">{{ $resource_string_slice }}</span>
            </div>
    ...
    <div class="notification-menu-button-container relative none">
        ...
        <div class="suboptions-container nested-soc simple-button-suboptions-container">
            ...
            <a href="" class="suboption-style-1 fs13 disable-switch-notification @if($disabled) enable-notification @else disable-notification @endif align-center" style="width: 230px;">
                ...
                @if($disabled)
                    <span class="button-text">{{ __('Enable notifications') }}</span>
                @else
                    <span class="button-text">{{ __('Disable notifications') }}</span>
                @endif
                ...
                <input type="hidden" class="disable-action-text" value="{{ __('Disable notifications') }}">
                <input type="hidden" class="enable-action-text" value="{{ __('Enable notifications') }}">
                <input type="hidden" class="notifications-enabled" value="{{ __('Notifications enabled') }}">
                <input type="hidden" class="notifications-disabled" value="{{ __('Notifications disabled') }}">
            </a>
            @endif
        </div>
    </div>
