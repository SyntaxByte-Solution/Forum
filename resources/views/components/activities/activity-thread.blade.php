<div class="activity-thread-wrapper thread-container-box relative" style="@if($is_ticked) background-color: #cfffcf21; border: 1px solid #a7cca7bd; @endif">
    ...
    <div class="absolute full-shadowed zi12 thread-permanent-deletion-dialog br6">
        ...
        <div class="white px8 py8 full-height flex-column full-center border-box">
            <h2 class="no-margin fs18">{{ __('Please make sure you want to delete permanently the discussion') }} !</h2>
            <p class="fs12 no-margin">{{ __('This will remove the discussion permanently. By removing the discussion, everything related to it will be deleted (replies, votes ..)') }}</p>
            <div class="mt8 flex align-center">
                <div>
                    <div class="simple-white-button bold destroy-thread-button" style="background-color: #d03535; color: white">{{ __('Delete permanently') }}</div>
                    <input type="hidden" class="thread-destroy-link" value="{{ route('thread.destroy', ['thread'=>$thread->id]) }}">
                    <input type="hidden" value="{{ __('Delete permanently') }}" class="btn-text-no-ing">
                    <input type="hidden" value="{{ __('Deleting') }}.." class="btn-text-ing">
                </div>
                <div class="spinner size17 opacity0" style="margin-left: 2px">
                    <svg class="size17" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 197.21 197.21"><path fill="#fff" d="M182.21,83.61h-24a15,15,0,0,0,0,30h24a15,15,0,0,0,0-30ZM54,98.61a15,15,0,0,0-15-15H15a15,15,0,0,0,0,30H39A15,15,0,0,0,54,98.61ZM98.27,143.2a15,15,0,0,0-15,15v24a15,15,0,0,0,30,0v-24A15,15,0,0,0,98.27,143.2ZM98.27,0a15,15,0,0,0-15,15V39a15,15,0,1,0,30,0V15A15,15,0,0,0,98.27,0Zm53.08,130.14a15,15,0,0,0-21.21,21.21l17,17a15,15,0,1,0,21.21-21.21ZM50.1,28.88A15,15,0,0,0,28.88,50.09l17,17A15,15,0,0,0,67.07,45.86ZM45.86,130.14l-17,17a15,15,0,1,0,21.21,21.21l17-17a15,15,0,0,0-21.21-21.21Z"/></svg>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="full-width">
            <div class="flex align-center space-between mb8">
                <div class="flex align-center">
                    ...
                    @if($is_ticked)
                        <svg class="size20 ml8" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                        ...
                        <div class="suboptions-container suboptions-container-right-style">
                            @if(is_null($thread->deleted_at))
                                <a href="{{ $thread->link }}" target="_blank" class="no-underline simple-suboption flex align-center">
                                    ...
                                    <div class="black">{{ __('See discussion') }}</div>
                                </a>
                                @can('update', $thread)
                            ...
                            @else
                                <div class="simple-suboption restore-thread-button flex align-center">
                                    ...
                                    <div class="btn-text">{{ __('Restore discussion') }}</div>
                                    ...
                                    <input type="hidden" class="restore-link" value="{{ route('thread.restore', ['thread'=>$thread->id]) }}">
                                    <input type="hidden" class="btn-text-no-ing" value="{{ __('Restore discussion') }}">
                                    <input type="hidden" class="btn-text-ing" value="{{ __('Restoring') }}..">
                                </div>
                                <div class="thread-permanent-delete simple-suboption flex align-center" style="background-color: #d03535; color: white;">
                ...
                <div class="flex align-center height-max-content mr8">
                    <div class="flex flex-column align-center">
                        ...
                        <svg class="size14" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">
                            <title>{{ __('UP') }}</title>
                            @upvoted($thread, 'App\Models\Thread')
                        ...
                        <p class="bold fs14 text-center votable-count no-margin">{{ $thread->votevalue }}</p>

                        <svg class="size14" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">
                            <title>{{ __('DOWN') }}</title>
                            @downvoted($thread, 'App\Models\Thread')
                ...
                <div>
                    ...
                    <div class="flex align-center mt8">
                        <div class="simple-border-container mr4">
                            ...
                            @endif
                            <p class="fs12 no-margin unselectable">{{ $thread->likes->count() }} {{__('likes')}}</p>
                        </div>
                        <div class="simple-border-container mr4">
                            ...
                            <p class="fs12 no-margin unselectable">{{ $thread->posts->count() }} {{__('replies')}}</p>
                        </div>
                    </div>
                </div>
            </div>
