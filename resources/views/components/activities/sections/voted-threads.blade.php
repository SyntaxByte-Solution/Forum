            ...
            <div class="full-center text-center" style="margin: 46px auto 0 auto; width: 80%;">
                <div class="flex flex-column align-center">
                    ...
                    @if(\Illuminate\Support\Facades\Auth::check() && auth()->user()->id == $user->id)
                        <p class="fs20 text-center bold my8 text-center" style="margin-bottom: 2px">{{ __("You have not voted any discussions yet") }} !</p>
                        <div class="flex">
                            <div class="flex align-center move-to-middle my4">
                                <p class="no-margin text-center">{{ __("To vote a discussion, click on the up or down buttons in the left side of discussion post") }}</p>
                            </div>
                        </div>
                    @else
                        <p class="fs20 text-center bold my4">{{ __("This user has no voted any discussion for the moment") }} !</p>
                    @endif
                </div>
            </div>
        @endif
