@if ($paginator->hasPages())
    ...
    <div class="flex align-center">
        ...
        @if (!$paginator->onFirstPage())
            <a href="{{ $previous_page_url }}" class="pagination-item pag-active br4-first" rel="prev" aria-label="@lang('pagination.previous')">Prev</a>
        @endif
            ...
            @if (is_array($element))
                @foreach ($element as $page => $url)
                    ...
                    @if ($page == $paginator->currentPage())
                        <a href="{{ $url }}" class="pagination-item pagination-item-selected" aria-current="page">{{ $page }}</a>
                    @else
        ...
        @if ($paginator->hasMorePages())
            <a href="{{ $next_page_url }}" class="pagination-item pag-active br4-last" aria-disabled="true" aria-label="@lang('pagination.next')">Next</a>
        @endif
    </div>
@endif
