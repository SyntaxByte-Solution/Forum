            ...
            <div class="full-center" style="margin-top: 46px">
                <div class="flex flex-column align-center">
                    ...
                    @if(\Illuminate\Support\Facades\Auth::check() && auth()->user()->id == $user->id)
                        <p class="fs20 text-center bold my8" style="margin-bottom: 2px">{{ __("You have not voted any discussions or question yet !") }}</p>
                        <div class="flex">
                            <div class="flex align-center move-to-middle my4">
                                <p class="no-margin">{{ __("To vote a thread, click on the up or down buttons in the left side of thread post") }}</p>
                            </div>
                        </div>
                    @else
                        <p class="fs20 text-center bold my4">{{ __("This user has no voted threads for the moment !") }}</p>
                    @endif
                </div>
            </div>
        @endif
