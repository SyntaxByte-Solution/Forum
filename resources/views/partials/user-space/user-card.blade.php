<div class="ms-right-panel mt8">
    @php
        $ustatus = Cache::has('user-is-online-' . $user->id) ? __('active') : __('inactive');
        $login_status = Cache::has('user-is-online-' . $user->id) ? __('online') : __('offline');
    @endphp
    @isset($withcover)
    <div class="flex ">
        <div class="relative us-user-media-container full-width">
            ...
            <div class="my8">
                <p class="fs12 gray no-margin">{{__('Join Date')}}: <span class="black">{{ (new \Carbon\Carbon($user->created_at))->toDayDateTimeString() }}</span></p>
                <div class="flex my8">
                    @php
        ...
        <div class="my4">
            <div class="flex align-center">
                <div class="flex align-center">
                    ...
                    <p class="inline-block my4 fs12">{{__('Discussions')}}: </p><span class="bold ml8">{{ $user->threads()->without(['category.forum','likes','posts', 'visibility', 'status', 'votes', 'user.status'])->count() }}</span>
                </div>
        ...
        <div class="my4">
            <div class="flex align-center">
                <div class="flex align-center">
                    ...
                    <p class="inline-block my4 fs12">{{__('Replies')}}: </p><span class="bold ml8">{{ $user->posts()->count() }}</span>
                </div>
            </div>
        </div>
        <div class="my4">
        ...
        <div class="my4">
            <div class="flex align-center">
                <div class="flex align-center">
                    ...
                    <p class="inline-block my4 fs12">{{__('Votes casts')}}: </p><span class="bold ml8">{{ $user->votes()->count() }}</span>
                </div>
            </div>
        </div>
    </div>
