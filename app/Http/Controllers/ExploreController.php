...
class ExploreController extends Controller
{
     ...
     * So this is relative to the number of threads posted per day, maybe started with 24 first and then
    ...
    public function explore() {
        ...
        $threads_builder = Thread::query()->without(['posts', 'votes', 'likes', 'user.status']);
        ...
        switch($sortby) {
            ...
            case 'votes':
                ...
                $threads = $threads->take(self::THREADS_ITERATION_LIMIT) 
                                    // ^ This is important for performence purposes because we use sortby votevalue which is not an attribute
                                    // and therfore we will return all the fetched threads to the memory. we have to limit the returned threads number
                                    // with a limit(200) in case the returned threads are greather than 200
                    ->orderBy('created_at', 'desc')
                    ->get()
        ...
        switch($indexes['sortby']) {
            case 'popular-and-recent':
                ...
                if($skip) {
                    ...
                    if(($threadscount=$threads->count()) < self::FETCH_PAGESIZE) { #1
                        ...
                        } while(($gapthreadscount=$gapthreads->count()) < $gap && $limit < 40);
                        $from-=$interval_increase_by;
                    ...
                    do {
                        ...
                        $from+=$interval_increase_by;
                    } while(($threadscount=$threads->count()) < self::FETCH_PAGESIZE && $limit < 40);
                    $from-=$interval_increase_by;
                    if($limit >= 40)
            ...
            case 'votes':
                if($skip) {
                    ...
                    $threads = 
                        $threads->where('created_at', '<=', Carbon::now()->subHours($to)->toDateTimeString())
                        ->where('created_at', '>=', Carbon::now()->subHours($from)->toDateTimeString())
                        ->orderBy('created_at', 'desc')
                        ->get()
                        ->sortBy(function($thread) {
