<div class="activities-section activities-archived-threads-section">
    <div class="activities-section-content-header flex">
        <h3 class="move-to-middle forum-color flex align-center unselectable">
            ...
            {{ __('Archived discussions') }} [<span class="current-section-thread-count mx4">{{ 10 * ($archivedthreads->count() / 10) }}</span> / <span class="current-section-global-threads-count mx4">{{ $user->archivedthreads->count() }}</span> ]
        </h3>
    </div>
    <div class="activities-section-content">
        ...
        @endforeach
        @if($user->archivedthreads->count() > 10)
        <div class="flex activity-section-load-more">
            <input type="hidden" class="section" value="archived-threads">
        ...
        @if(!$archivedthreads->count())
            <div class="full-center" style="margin-top: 46px">
                <div class="flex flex-column align-center">
                    ...
                    <p class="fs20 text-center bold my8">{{ __("You don't have any archived discussions") }}.</p>
                </div>
            </div>
        @endif
    </div>
