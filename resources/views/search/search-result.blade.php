...
@section('content')
    ...
    <div id="middle-container" class="middle-padding-1 flex">
        <div class="full-width">
            <div class="flex">
                <div>
                    <h1 id="page-title" class="my8 fs28 forum-color">{{ __('Explore User & Threads') }}</h1>
                </div>
            </div>
            <div>
            ...
            <div>
                @if($users->count())
                    <div class="flex space-between align-center">
                        <h2 class="fs20 blue unselectable my4 flex align-center">{{ __('Users') }}<span class="gray fs14 ml4 @if($search_query == '') none @endif">({{$users->total()}} {{__('found')}})</span></h2>
                        @if($users->count() > 4)
